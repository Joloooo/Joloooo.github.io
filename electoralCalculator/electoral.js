...
const mirrorSeats = () => {
  ...
  dynamicResults.forEach((result, index) => {
    ...
    if (!isNaN(percentValue)) {
      if (percentValue < 5) {
        seatNumber.innerText = "did not cross 5% threshold";
        totalSeats.innerText = "did not cross 5% threshold";
        gainedSeats.innerText = "did not cross 5% threshold";
...
const createdynamic = () => {
  ...
  newDynamic.innerHTML = `
    <label id="partyLabel">Party <input type="text" class="partyInput" id="${partyInputID}"></label>
    <label id="percentLabel">Percent (%) <input class="percentInput" id="${percentInputID}" type="number"></label>
  `;
